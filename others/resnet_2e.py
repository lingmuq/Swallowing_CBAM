...
class ResNet(nn.Module):
    ...
    def _make_layer(self, block: Type[Union[BasicBlock, Bottleneck]], planes: int, blocks: int, stride: int = 1, dilate: bool = False) -> nn.ModuleList:
        norm_layer = self._norm_layer
        downsample = None
        previous_dilation = self.dilation
        if dilate:
    ...
    #     x = self.relu(x)
    #     x = self.maxpool(x)
    #     feature_a, feature_a_cbam = self.layer1(x)
    #     feature_b, feature_b_cbam = self.layer2(feature_a)
    #     feature_c, feature_c_cbam = self.layer3(feature_b)
    #     feature_d, feature_d_cbam = self.layer4(feature_c)
    #     return [feature_a, feature_b, feature_c], [feature_a_cbam, feature_b_cbam, feature_c_cbam]
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        ...
        x = self.relu(x)
        x = self.maxpool(x)
        feature_a, feature_a_cbam = self._forward_layer(x, self.layer1)
        feature_b, feature_b_cbam = self._forward_layer(feature_a, self.layer2)
        feature_c, feature_c_cbam = self._forward_layer(feature_b, self.layer3)
        feature_d, feature_d_cbam = self._forward_layer(feature_c, self.layer4)
        return [feature_a, feature_b, feature_c], [feature_a_cbam, feature_b_cbam, feature_c_cbam]
