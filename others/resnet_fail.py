...
class ResNet(nn.Module):
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        ...
        feature_a_cbam = self.cbam(feature_a)
        feature_b_cbam = self.cbam(feature_b)
        feature_c_cbam = self.cbam(feature_c)
...
# 多分これ
# base_width（Bottleneck内の最初のconv1x1の出力チャネル数）がresnet50の2倍らしいけどわかんないのでlossのとこでprint(f"a[{n}].shape: {a[n].shape}する
def wide_resnet50_2(pretrained: bool = False, progress: bool = True, **kwargs: Any) -> ResNet:
    ...
    return _resnet('wide_resnet50_2', Bottleneck, [3, 4, 6, 3], pretrained, progress, **kwargs), BN_layer(AttnBottleneck,3,**kwargs)    # ResNetではBottleneck, BN_layerではAttnBottleneck
