...
import matplotlib
import pickle
from sklearn.metrics import f1_score  # 一开始就导入
...
def evaluation(encoder, bn, decoder, dataloader, device, _class_=None):
    ...
    # -------- 新增部分：计算Best Threshold和Best F1 ----------
    gt_array = np.array(gt_list_px)
    pr_array = np.array(pr_list_px)

    best_f1 = 0
    best_th = 0
    for th in np.linspace(0, 1, 100):  # 你可以把100改成更高比如200更细腻
        pred = (pr_array >= th).astype(int)
        f1 = f1_score(gt_array, pred)
        if f1 > best_f1:
            best_f1 = f1
            best_th = th
