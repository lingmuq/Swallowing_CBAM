    ...
    def load_dataset(self):
        ...
        for defect_type in defect_types:
            if defect_type == 'good':
                ...
                img_paths = [p for p in img_paths if p.endswith(('.png', '.bmp'))]  # 只取图片
                ...
                gt_paths = glob.glob(os.path.join(self.gt_path, defect_type, '*.png'))  # ground truth通常是png
                img_paths.sort()
                gt_paths.sort()

                # 按文件名前缀对应
                img_basenames = [os.path.splitext(os.path.basename(f))[0] for f in img_paths]
