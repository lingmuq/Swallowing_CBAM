...
image_size = 256
batch_size = 1
class_list = ['bread'] # 一括でやると時間かかるのでいちいち変える
...
for _class in class_list:
    ...
    ckp = torch.load('./checkpoints/wres50_bread.pth')  # ここもいちいち変える
    ...
    with torch.no_grad():
        ...
        precision, recall, thresholds = precision_recall_curve(gt_array.flatten(), pr_array.flatten())  # F1最適閾値
    ...
    if not os.path.exists('./img/'+_class):
        os.makedirs('./img/'+_class)
    if not os.path.exists('./img/'+_class+'/pred'):
        os.makedirs('./img/'+_class+'/pred')
    if not os.path.exists('./img/'+_class+'/gt'):
        os.makedirs('./img/'+_class+'/gt')
    if not os.path.exists('./img/'+_class+'/hmap'):
        os.makedirs('./img/'+_class+'/hmap')
    ...
    for img, gt, label, _ in test_dataloader:
        ...
        # score_vis = np.power(score, 0.5)
        # heatmap = cv2.applyColorMap(np.uint8(score_vis * 255), cv2.COLORMAP_JET)
        ...
        cv2.imwrite('./img/'+_class+'/pred/'+str(count)+'_'+'ad.png', img_pred)
        ...
        cv2.imwrite('./img/'+_class+'/gt/'+str(count)+'_'+'ad.png', img_gt)

        # heatmap
        # heatmap = (heatmap - np.min(heatmap)) / (np.max(heatmap) - np.min(heatmap) + 1e-8) *255
        cv2.imwrite('./img/'+_class+'/hmap/'+str(count)+'_'+'ad.png', heatmap)
        ...
        # cv2.imwrite('./img/'+_class+'/'+str(count)+'_'+'ad.png', ano_map)
