...
def evaluation(encoder, decoder, dataloader,device,_class_=None):    
...
def test(_class_):
    ...
    auroc_px, auroc_sp, aupro_px = evaluation(encoder, decoder, test_dataloader, device,_class_)
...
def vis_nd(name, _class_):
    ...
    encoder, _ = resnet18(pretrained=True)
    encoder = encoder.to(device)
    # bn = bn.to(device)
    encoder.eval()
...
def detection(encoder, decoder, dataloader,device,_class_):
    # _, t_bn = resnet50(pretrained=True)
    # bn.load_state_dict(bn.state_dict())
    # bn.eval()
    # t_bn.to(device)
