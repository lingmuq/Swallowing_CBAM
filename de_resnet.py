...
class ResNet(nn.Module):
    ...
    def _make_layer(self, block: Type[Union[BasicBlock, Bottleneck]], planes: int, blocks: int, stride: int = 1, dilate: bool = False) -> nn.Sequential:
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            upsample = nn.Sequential(
                deconv2x2(self.inplanes, planes * block.expansion, stride), norm_layer(planes * block.expansion)
            )
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        feature_a = self.layer1(x)          # [16, 2048, 8, 8]   -> [16, 1024, 16, 16]
        feature_b = self.layer2(feature_a)  # [16, 1024, 16, 16] -> [16, 512, 32, 32]
        feature_c = self.layer3(feature_b)  # [16, 512, 32, 32]  -> [16, 256, 64, 64]
    
        return [feature_c, feature_b, feature_a]    # S(3), S(2), S(1)のlist
