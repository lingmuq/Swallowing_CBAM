...
def get_data_transforms(size, isize):
    ...
    data_transforms = transforms.Compose([
        ...
        # transforms.CenterCrop(isize - 32),
        # transforms.CenterCrop(isize - 60),
        # transforms.CenterCrop(isize - 64),
...
# PyTorchのDatasetを継承 -> SwallowingDatasetを扱う
class SwallowingDataset(torch.utils.data.Dataset):
    ...
    def load_dataset(self): # パスとラベルのロード
        img_tot_paths = []
        gt_tot_paths = []
        tot_labels = []
        tot_types = []
        ...
        for defect_type in defect_types:
            if defect_type == 'good':
                ...
                img_paths = [p for p in img_paths if p.endswith(('.png', '.bmp'))]

                img_tot_paths.extend(img_paths)
                gt_tot_paths.extend([0] * len(img_paths))   # 0（正常）で埋める
                ...
                img_basenames = [os.path.splitext(os.path.basename(f))[0] for f in img_paths]   # 拡張子を除く
                gt_basenames = [os.path.splitext(os.path.basename(f))[0] for f in gt_paths]

                assert img_basenames == gt_basenames, f"Image names and GT names do not match in defect type: {defect_type}"

                img_tot_paths.extend(img_paths) # パス追加
                gt_tot_paths.extend(gt_paths)

                tot_labels.extend([1] * len(img_paths)) # 1（異常）で埋める
    ...
    def __getitem__(self, idx): # インデックスに対応するデータを返す
...
def load_data(dataset_name='mnist',normal_class=0,batch_size='16'):
    ...
    if dataset_name == 'cifar10':
        img_transform = transforms.Compose([transforms.Resize((32, 32)), transforms.ToTensor(), transforms.Normalize(mean=(0.485, 0.456, 0.406), std=(0.229, 0.224, 0.225))])
    ...
    # Retina(OCT画像)
        ...
        orig_transform = transforms.Compose([transforms.Resize([128, 128]),transforms.ToTensor()])
